...
#define OZ_BADPARAM 99
#define OZ_ENDOFFILE 25
#define OZ_SUCCESS 1

static char *inputbuff = "0,19508,0";

uLong getinput (void *param, uLong *size, const char **buff);
...
{
  int rc;
  uLong sts, nargs, uncnv;
  unsigned int v1, v2, v3;

  sts = oz_sys_xscanf (getinput, NULL, &nargs, &uncnv, "%u,%u,%u", &v1, &v2, &v3);
  printf ("sts=%u, nargs=%u, uncnv=%u, v1,v2,v3=%u,%u,%u\n", sts, nargs, uncnv, v1, v2, v3);
  return (0);
}
...
{
  if (inputbuff == NULL) return (OZ_ENDOFFILE);
  *size = strlen (inputbuff);
  *buff = inputbuff;
  inputbuff = NULL;
  return (OZ_SUCCESS);
}
...
#define ARG(__type) ((argpos == 0) ? va_arg (ap, __type) : (__type)(arg[argpos]))
...

{
  ...
  int argpos, base, c, done, flags, number_signed, skip_space, strsize, width;
  Pblk pblk;
  void **arg;
  ...
  while ((fmtc = *(fmt ++)) != 0) {
    ...
    if (ISDIGIT (fmtc)) {
      ...
      if (fmtc == '$') {
        ...
        argpos = 0;				/* pretend like we never saw it */
