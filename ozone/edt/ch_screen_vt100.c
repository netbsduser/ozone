...
genloop:
  if (line2 == NULL) {							/* check for [EOB] */
    ...
    if (columnumber < 0) columnumber = 0;				/* never shifted left off screen */
...

{
  ...
  strp   = representation (c, temp, columnumber);			/* get pointer to the rendition */
  strl   = strlen (strp);						/* get length of the rendition */
  newcol = columnumber + strl;						/* get what column would be after outputting */
  if (columnumber < 0) {						/* see if we're 'shifted left' */
    strp -= columnumber;						/* if so, lop off that much from start of string */
    strl += columnumber;
    columnumber = 0;							/* if anything's left, it goes in first column */
  }
  ...
  /* Save the new zero-based 'virtual' cursor position.  Negative if still */
  /* in 'shiftleft' area.  Gt ch_screen_width if we output a diamond.      */
