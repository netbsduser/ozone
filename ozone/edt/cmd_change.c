...
int ch_screen_shiftleft = 0;	/* - how many chars to leave off of beg of line */
				...
				/* these three are set by ch_screen_init: */
...
int ch_screen_width;		/* - number of columns on screen */

int autoshift;			/* 0 : no autoshifting */
...

{
  ...
  while (!ch_exited) {				/* repeat until we exit change mode */
    ...
    if ((autoshift != 0) && (cur_position.line != NULL)) {
      columnumber = ch_screen_chr2col (cur_position.offset, string_getval (line_string (cur_position.line)));
      while (ch_screen_shiftleft > columnumber) {
        ch_screen_shiftleft -= autoshift * 8;
        if (ch_screen_shiftleft < 0) ch_screen_shiftleft = 0;
      }
      while (ch_screen_shiftleft + ch_screen_width <= columnumber) ch_screen_shiftleft += autoshift * 8;
    }
